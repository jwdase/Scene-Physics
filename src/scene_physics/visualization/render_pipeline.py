...
def render_camera_views(
    ...
    for i, (az, el, dist, fov, desc) in enumerate(_CAMERA_VIEWS, start=1):
        ...
        intr = CameraIntrinsics(
            width=width, height=height, fov_degree=fov,
            eye=eye, target=_VIEW_TARGET.copy(),
        )
...
def _make_grid_montage(out_dir: Path, names, fovs, descs, rows, cols, tile=(192, 144)) -> Path:
    ...
    for idx, name in enumerate(names):
        ...
        draw.text((c * tw + 3, r * th + 2), f"fov {fovs[c]}", fill=(255, 235, 90), font=font)
        if c == 0:
            draw.text((3, r * th + th - 11), descs[r][:20], fill=(120, 220, 255), font=font)
...
def render_fov_grid(
    ...
    scene_dir = Path(scene_dir)
    usd = next(scene_dir.glob("data/*_physics.usdc"))
    out_dir = Path(out_dir) if out_dir is not None else scene_dir / "camera_views" / "fov_grid"
    ...
    for pi, (az, el, dist, _fov, desc) in enumerate(_CAMERA_VIEWS, start=1):
        ...
        for fi, fov in enumerate(_FOV_VALUES, start=1):
            ...
            views.append(d)
            names.append(oid)
            (cfg_dir / f"{oid}.json").write_text(json.dumps({
                "id": oid,
                "position": {"index": pi, "desc": desc,
                             "azimuth_deg": az, "elevation_deg": el, "distance_m": dist},
                "fov_degree": fov,
                "width": width, "height": height,
                "eye": d["eye"], "target": d["target"], "up": d["up"],
            }, indent=2))
    ...
    blender_runner.run_render_views(job)
    montage = _make_grid_montage(out_dir, names, _FOV_VALUES, descs, rows=len(_CAMERA_VIEWS), cols=len(_FOV_VALUES))
    print(f"[fov-grid] {scene_dir.name} -> {out_dir} ({len(names)} options; montage: {montage})")
    return out_dir
