...
# Material Properties for Dynamic Objects (Rectangle & Pentagon)
DYNAMIC_FRICTION = 0.9      # Coefficient of friction (mu) - higher = more friction
...
STATIC_FRICTION = 0.8       # Friction for table surface
...
PENTAGON_SCALE = 1.0        # Scale factor for pentagon
...
PENTAGON_POSITION = (0., 0., 0.)    # Use OBJ file's embedded position
...
# Output Settings
OUTPUT_VIDEO = "recordings/stable_scene_simulation.mp4"  # If MP4 fails, try .avi or .gif
...
# Add Pentagon (DYNAMIC)
pentagon = MeshBody(
    builder=builder,
    body=pentagon_path,
    solid=True,
    scale=PENTAGON_SCALE,
    position=wp.vec3(*PENTAGON_POSITION),
    mass=1.0,  # Mass will be calculated from density and volume
    material=dynamic_material,
)
...
bodies = [table, rectangle, pentagon]
