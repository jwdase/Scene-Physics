...
# Type aliases (originally from b3d.types, but without genjax dependency)
...
# Camera functions (from b3d.camera)

def pixel_centers_from_shape(img_shape: ImageShape):
    """
...
def camera_from_screen(uv: ScreenCoordinates, intrinsics):
    z = jnp.ones_like(uv.shape[-1:])
    return camera_from_screen_and_depth(uv, z, intrinsics)


def camera_from_depth(z: DepthImage, intrinsics):
    """
...
def screen_from_camera(xyz: CameraCoordinates, intrinsics):
    """
...
def camera_matrix_from_intrinsics(intr: Intrinsics):
    """
...
def camera_rays_from_intrinsics(intr: Intrinsics):
    """
...
def canvas_from_intrinsics(intr: Intrinsics):
    """
